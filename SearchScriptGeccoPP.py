...
# 		print (r)
	...
	pp_dbgap_join = """select sm.biosample_accession, sp.sex dbgap_sex, json_extract_scalar(phenopacket, '$.subject.sex') pp_sex FROM dbgap_demo.scr_gecco_susceptibility.subject_phenotypes_multi sp join dbgap_demo.scr_gecco_susceptibility.sample_multi sm on sm.dbgap_subject_id = sp.dbgap_subject_id join sample_phenopackets.ga4gh_tables.gecco_phenopackets pp on pp.id = sm.biosample_accession"""

	query_job = searchClient.runQuery(dbgap_query3)  # Send the query
	print(len(query_job))
# 	for r in query_job:
# 		print (r)
